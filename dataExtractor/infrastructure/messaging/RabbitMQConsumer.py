...
class RabbitMQConsumer:
    ...
    def _callback(self, ch: pika.channel.Channel, method: pika.spec.Basic.Deliver,
        ...
        with (self._uow.open_session() as session):
            ...
            model_vehicles: List[Vehicle] = []
            for vehicle in frame_message.vehicles:
                model_vehicle = session.query(Vehicle
                                              ).filter(Vehicle.external_id == vehicle.external_id).one_or_none()
                if model_vehicle is None:
                    model_vehicle = Vehicle(vehicle.external_id, [])
                    session.add(model_vehicle)
                else:
                    model_vehicle.regions.extend(vehicle.regions)
                model_vehicles.append(model_vehicle)

            for service in self._services:
                service.process(model_vehicles, frame_message.frames)
